...
#//*[@id="mw-content-text"]/aside/figure/a



#driver = 크롬(800,10,False)

식별 = str (int (random()*1000))

r = 읽기("myanime만화주소",'\n','')
#p = re.compile (  '(https://myanimelist.net/anime/)(\d*)(/.*)'  )
#p = re.compile (  '(https://myanimelist.net/anime/)(\d*)/(^[/]*)(/.*)'  )
p = re.compile (  '(https://myanimelist.net/anime/)(\d*/)([^/]*)(.*)'  )
...
def 위치추출(검색어):
   l = []
   b = 뽀글('https://myanimelist.net/search/all?q='+검색어)
   z = b.find( "div", {"class":"js-scrollfix-bottom-rel"} )
   z = b.find( "article" )
   z = b.findAll( "a", {"class":"hoverinfo_trigger fw-b fl-l"} )

   for i in z:
      t1 = re.sub(p,r'\2',i['href'])
      t2 = re.sub(p,r'\3',i['href'])
      l.append('https://myanimelist.net/anime/' + t1 + t2 + '/characters')

   이어쓰기 ("만화별 캐릭터 주소 5추가.txt","\n\n이하 "+검색어+'\n')
   이어쓰기 ("만화별 캐릭터 주소 5추가.txt",l)

a = []
for i in r:
   a.append( re.sub(p, r'\3',i) )

덮어쓰기 ("조사한 애니메이션.txt",a)
#
# ---- PFW 솔루션 ----
#F = ''.join( 읽기("aaaa") )
#P = re.compile("")
