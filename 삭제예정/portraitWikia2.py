...
if True:                                    # 응용 함수 리스트
  ...
  def 새폴더(경로에):                             # 폴더가 없으면 새로 만들어지고, 이미 있으면 그냥 넘어갑니다
     try:
         os.mkdir(경로에)
  ...
  def 다운(경로에, 파일URL):                       # 파일의 URL에 접근하고, 접근한 파일의 비트를, 로컬 경로에 복제합니다
  ...
  def 읽기(경로, 찾아, 바꾸기):                              # 파일로부터 필사된 결과물을 리스트로 리턴합니다
  ...
  def 덮어쓰기(경로에, 변수):                       # 넘겨받은 내용을 파일 경로에 문자열로 출력합니다 (덮어쓰기)

     with open(경로에, 'w', encoding="utf-8") as A:
  ...
  def 이어쓰기(경로에, 변수):                       # 넘겨받은 내용을 파일 경로에 문자열로 출력합니다 (기존 내용에 이어붙이기)

     with open(경로에, 'a', encoding="utf-8") as A:
  ...
  def 크롬(가로, 세로, 투명모드):
      ...
      if 투명모드:
         ...
         print ("투명모드(headless)로 실행합니다")

      return webdriver.Chrome("C:\\Users\\채은\\Desktop\\파이썬\\chromedriver", chrome_options=ops)
...
def 캐릭마다주소얻기(url):
   ...
   z = bs.find("div", {"class":"js-scrollfix-bottom-rel"} )
   zz = z.findAll("a", {"href":re.compile(r'.+character\/.+')})

   lst = set()
   
   for i in zz:
     lst.add (i['href']+"/pictures")

   lst = list(lst)


   if list(lst)==[]:
      이어쓰기("undone1.txt", url + "ㄴ빔\n")

     
   try:
     이어쓰기("charadds1.txt", list(lst))
     이어쓰기("done1.txt", url + "\n")
     
   except:
     print ("예외 발생")
     이어쓰기("undone1.txt", url + "\n")

def 캐릭마다파일주소(url):
   html = 소스보기 (url)
   bs = BeautifulSoup(html, "html.parser")

   z = bs.find ("table", {"align":"center"} )

   try:
     zz = z.findAll ("a", {"href":re.compile(r'.+characters.+')} )
     ...
     try:
       이어쓰기("imgAdrs2.txt", lst)
       이어쓰기("doneB.txt", url + "\n")
     
     except:
       print ("예외 발생")
       이어쓰기("undoneB.txt", url + " ㄴ예외 발생\n")
       
   except:
     print ("사진 없음")
     이어쓰기("undoneB.txt", url + " ㄴ사진 없음\n")


print (중복제거("머 머머머 머 꺅", "\n"))
#
# ---- PFW 솔루션 ----
#F = ''.join( 읽기("aaaa") )
#P = re.compile("")
#덮어쓰기("aaaa",re.sub(P,"",F) )
