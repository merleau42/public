...
def thirty(subsoup, i):
	#ret = subsoup.select_one('input + colgroup + tbody').select(f'tr+tr td:nth-child({i})')
	ret = MAP(lambda x : x.select_one(f'td:nth-child({i})'), subsoup.select('.list0, .list1'))
	return ret

BBS1 = [	['forum', 16],	['tips', 192],	['novel', 324],	['report', 567],	['sos', 1229]	]
BBS2 = [	['adver', 91],	['notice', 12],	['forum2', 13]	]
BBS3 = [	['qna', 4254],	['free', 14407]	]
BBS4 = [	['ego', 15],	['wisdom', 9]	]
BBS5 = [	['screenshot', 114]				]

for target in BBS5[0:1]:
	bbs = target[0]
	pgs = target[1]

	for page in range(1,pgs):
		#if page<0 : continue

		url = f'https://www.mabinogi.pe.kr/bbs/zboard.php?id={bbs}&page={page}'

		r = requests.get(url, headers=헤더)
		time.sleep(0.5)
		s = 끓이기(r.text)

		#실제 URL상 글번호 / 게시판 표기상 글번호
		진번 = MAP(lambda x: re.search( r'no=(\d+)', x.a['href'] )[1], thirty(s, 2))
		가번 = MAP(lambda x: x.text, thirty(s, 1))

		#분류 아이콘
		icon = "000000000000000000000000000000"
		#icon = MAP(lambda x: okay(x.div)["class"][0], thirty(s, 2))

		#분류 카테고리
		카테 = "000000000000000000000000000000"
		#카테 = MAP(lambda x: x.text, thirty(s, 2))

		#제목, 댓글 달린 갯수, 작성자, 작성일, 조회수
		제목 = MAP(lambda x: re.sub(r'\n|  +', '', x.text), thirty(s, 2))
		# 제목 = MAP(lambda x: okay(x.select_one('.list_han')).text, thirty(s, 3))
		댓수 = MAP(lambda x: int("0"+re.sub(r'[\[\]]','',x.select_one('.list_eng').text)), thirty(s, 2))
		쓰니 = MAP(lambda x: x.text, thirty(s, 3))
		날짜 = MAP(lambda x: x.text, thirty(s, 4))
		조회 = MAP(lambda x: x.text, thirty(s, 5))

		덮어쓰기(f'D:/크롤링/마탐즈/listpage/{bbs}/list_page_{bbs}{page}.html', r.text)
		for i in range(0,30):
			try:
				line =  f'{진번[i]}\t{가번[i]}\t{icon[i]}\t{카테[i]}\t{제목[i]}\t{댓수[i]}\t{쓰니[i]}\t{날짜[i]}\t{조회[i]}'
				이어쓰기( f'D:/크롤링/마탐즈/{bbs}_lines.txt', line )
			except:
				print (f'{i}줄 만 기록됨:', end='\t')
				break
 
		print( len(r.text), bbs, page )