...
def 연예인(소속사, k):
   html = 셀래 (driver, 1, "https://search.naver.com/search.naver?query=" + 소속사 + "소속 연예인")
   
   while True:
      bs = BeautifulSoup(html, "html.parser")
      z = bs.findAll("a", {"class":"name"} )
      s = []
      ...
      try:
         driver.find_element_by_xpath('//*[@id="pagination_dss_other_management_people"]/span[3]/button').click()
         sleep(1)
         html = driver.page_source
      except:
         s.append("\n")
         이어쓰기( "소속사 별 연예인_" + str(k) + ".txt", s )
         break
