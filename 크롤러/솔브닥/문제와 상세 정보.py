...
lst = []
for i in 불러오기('D:\크롤링\솔브닥\풀문제1', list):
	lst.append( re.search( r'(^.+?)\t' , i)[1] )

DRIVER = 구동("https://solved.ac/search?query=1000")
...
for problem in lst:
	...
	DRIVER.get(f"https://solved.ac/search?query={problem}")
	time.sleep(20)

	a = DRIVER.execute_script(f"return document.querySelector(`[href*='/problem/{problem}'] img`)['alt']")
	b = DRIVER.execute_script(f"return document.querySelector(`[href*='/problem/{problem}'] .__Latex__`).innerText")
	c = f'{problem}\t{a}\t{b}'
	print (c)
	이어쓰기('D:\크롤링\솔브닥\풀문제2', c)