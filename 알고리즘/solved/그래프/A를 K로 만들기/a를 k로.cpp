...
int main() {
	...
	while(T--){
		// 시작점과 목적지를 입력
		cin >> Start.y; Start.x=1;
		cin >> Final.y; Final.x=1;
		r= 1000001;
		// 배열 초기화
		init();
...
void BFS(queue<Point> &reserve){
	...
	while ( !reserve.empty() ) {
		...
		if ( !insane(next1) && dist[next1.y][next1.x] == -1 ){
			dist[next1.y][next1.x] = dist[current.y][current.x] + 1;
			depth = dist[next1.y][next1.x];

			reserve.push( next1 );
		}


		if ( !insane(next2) && dist[next2.y][next2.x] == -1 ){
			dist[next2.y][next2.x] = dist[current.y][current.x] + 1;
			depth = dist[next2.y][next2.x];
