...
/*   Updated: 2024/06/23 20:50:16 by keunykim         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	while (shovel > 0) // 파일끝(0) 오류발생(-1) 개행문자발견시(\n) 퍼오기 종료.
	{
		...
		free(storage);
		storage = tmp;
		if (ft_strchr(cart, NL)) // 방금 부어놓고 온 카트에 개행문자가 있었으면 종료.
			break;
	...
	if (shovel == -1) // 오늘 일진이 사나웠다면, 이 현장 다시 안옴.
		return (NULL);
	if (ft_strchr(storage, NL)) // 저장고에 개행문자가 있다면, len = 개행문자까지 길이
		len = ft_strchr(storage, NL) - storage + 1;
	else
