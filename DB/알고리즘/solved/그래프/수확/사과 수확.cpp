...
void draw(vector<vector<int>> &Graph, int beg=0) {
	cout << "\nㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡ\n";
	for (int i=beg; i<v+beg; i++) {
		cout << "노드 " << i << '\t';
		cout << "방문 " << vist[i] << '\t';
		cout << "순위 " << Rank[i] << '\t';
		cout << "거리 " << dist[i] << '\t';
