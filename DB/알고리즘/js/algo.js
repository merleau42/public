...
완전탐색 = (자료={}, 전략={}, 상태={}, 초기화=false) => {
	if (!초기화) {
		자료 = { 하한: [], 상한: [], ...자료 };
		전략 = { 필터: ()=>true, 액션: 상태=>[...상태.스택], 범위: ()=>[], 가지치기: ()=>true, 종료: ()=>false, ...전략 };
		상태 = { 스택: [], 깊이: 0, 해집합: [], ...상태 };
	}

    if (자료.하한[상태.깊이] == undefined) {
        if (전략.필터(상태, 자료))
            상태.해집합.push(전략.액션(상태, 자료));
        return;
    }

    for (e of 전략.범위(상태, 자료)) {
        if (전략.가지치기(상태, 자료)) {
            상태.스택.push(e);
			상태.깊이++;
            완전탐색(자료, 전략, 상태, true);
...
//! 조합론
cartesian = (하한, 상한, 전략={}) => 완전탐색({하한, 상한}, {범위: (상태, 자료) => range(자료.하한[상태.깊이], 자료.상한[상태.깊이] + 1), ...전략});
...
전략 = {
	필터: 상태 => {[i, j] = 상태.스택; return [i, j, i*j].includes( +input() )},
	종료: 상태 => 상태.해집합.length == 1,
};

cartesian([2,1], [9,9], 전략).length.log()
