...
//	ㅏ하노빨빨빨빨빨빨빨빨
//	 \ㅗㅗㅗㅗㅗㅗㅗㅗㅗㅗㅡ입원
log(  (확진 <= 50) && (입원 <= 10) ? 'White' : (30 < 입원) ? 'Red' : 'Yellow'  );


//: 30017
// 패티 == 치즈 	---> 패티 + (치즈-1)	---> 2min - 1
// 패티  > 치즈 	---> (치즈+1) + 치즈	---> 2min + 1
// 패티  < 치즈 	---> 패티 + (패티-1)	---> 2min - 1
//> 구성 요소간 대소 비교
log(  2* min(패티, 치즈)-1 + 2*(패티>치즈)  );

// 패티-1  == 치즈	---> 2치즈 + 1			---> 2min(패-1,치) + 1
// 패티-1   > 치즈	---> 2치즈 + 1			---> 2min(패-1,치) + 1
// 패티-1	< 치즈  ---> 2(패티-1) + 1		---> 2min(패-1,치) + 1
//> 의미 기반의 대소 비교
log(  2* min(패티-1, 치즈) + 1 )
...
K: 커피 머신을 설치한 층
N(i): i층의 사람 수
abs(A-B): A층에서 B층까지 이동 거리

D(i): i층의 사람들이 K층을 다녀가는 왕복 거리 == N(i) * 2abs(i-K)
(D1 + D2 + D3 + ...) 시그마 값이 최소가 되는 K를 구하기.
*/
seq = input('\n').map(Number);
...
//> 서로 모두 다름: ( +하나 +하나 -하나 == 0 ) 등등 다양한데, 불가능한 조합이거나,
//>	가능한 조합은 a < b < c 조건에서 (a+b == c)로 환원됨.
//~		(a+b == c)
