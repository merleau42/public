...
int	expose_win1(void *p){ (void)p; mlx_put_image_to_window(mlx,win1,img3,0,0); return 0;}
int	expose_win2(void *p){ (void)p; mlx_put_image_to_window(mlx,win2,img4,0,0); mlx_put_image_to_window(mlx,win2,img2,0,0); return 0;}

int	key_win1(int key,void *p){ (void)p; printf("Key in Win1 : %d\n",key); if (key==0xFF1B) exit(0); return 0;}
int	key_win2(int key,void *p){ (void)p; printf("Key in Win2 : %d\n",key); if (key==0xFF1B) exit(0); return 0;}
int	key_win3(int key,void *p){ (void)p; printf("Key in Win3 : %d\n",key); if (key==0xFF1B) mlx_destroy_window(mlx,win3); return 0;}

int	mouse_win1(int button,int x,int y, void *p){ (void)p; printf("Mouse in Win1, button %d at %dx%d.\n",button,x,y); return 0;}
...
int	main()
{
	...
	message(1, "\n     이미지 객체1 채우기");
		color_map_2((unsigned char *)data1,bpp1,sl1,42,42,endian1, 1);
		printf("  //  pixmap : %d",
					(int)((t_img *)img1)->pix);
	...
	message(1, "\n     이미지 객체3 채우기");
		color_map_2((unsigned char *)data3,bpp3,sl3,242,242,endian3, 1);
	...
	message(2, "\n\n#### 두번째 창 생성");
		...
		color_map_2((unsigned char *)data4,bpp4,sl4,242,242,endian4, 2);
